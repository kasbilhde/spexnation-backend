...
async function generateOrderPdf(data, orderID) {
    ...
    // PRESCRIPTION TABLE
    const rxHead = [["Eyes", "SPH", "CYL", "Axis", "ADD", data?.hasData[0]?.pdType == "1" ? "S-PD" : "D-PD"]];
    ...
    if (data?.iscoupon) {
        ...
        const totalPriceWithQuantity = totalPrice * data.hasData[0].quantity;
        doc.text(`£${totalPriceWithQuantity.toString()}`, pageW - 40, y, { align: "right" });
